...
- **[2026-02-15] Today 底部一体化调整**：移除中心页底部动作区分割框线，底部与主区改为同层一体化视觉。
  - Why：按最新反馈“不要框，一体化”，减少割裂感。
  - Impact：`app/globals.css`、`app/today/page.tsx`。
  - Verify：中心页底部不再出现独立上边框，交互逻辑保持不变。

- **[2026-02-15] 按钮比例修正**：将添加任务 `+` 按钮提升到 44x44，并统一主按钮高度为 44（开始/结束一致）。
  - Why：修复 `+` 与主按钮比例悬殊问题。
  - Impact：`app/globals.css`、`app/today/page.tsx`。
  - Verify：`+` 与主按钮视觉比例更平衡，仍保持圆角胶囊风格。

## Status / Next（增量）

- **[2026-02-15] 当前状态**：本轮样式改动已推送至 `85ff00c`，大陆测试机 `C:\www\pomlist` 重部署完成（PM2 id 15）。
- **[2026-02-15] 下一步**：如需，我可以继续把右侧 `Tasks` 页的按钮比例同步到同一套节奏。

## Decisions（增量）

- **[2026-02-15] Today 前后面板颜色统一**：主页中心面板背景与顶部标题栏改为和其余面板一致的深色层级。
  - Why：解决中心页与上/下/右页切换时前后色差明显的问题。
  - Impact：`app/globals.css`。
  - Verify：上滑/下滑/右滑切页时，中心页与相邻页主背景颜色一致。
...
- **[2026-02-15] 当前状态**：颜色统一已完成并通过本地全量校验。
- **[2026-02-15] 下一步**：按流程在大陆测试 VPS 重部署并验收切页观感一致性。

## Decisions（增量）

- **[2026-02-15] 大陆机重部署完成（前后面板颜色统一）**：按既定 Windows 部署流程完成发布并恢复 `pomlist` 进程。
  - Why：该改动属于线上视觉主流程，按约定需在测试 VPS 验收。
  - Impact：大陆机 `C:\www\pomlist`，进程 `pomlist`（PM2 id 16）。
  - Verify：`git rev-parse --short HEAD` 为 `358c6bc`；`pm2 ls` 显示 `pomlist` online；`curl -I http://127.0.0.1:3005/today` 返回 `HTTP/1.1 200 OK`。
...
- **[2026-02-15] Today 顶部进度区按会话显隐**：中心页顶部进度条与 `x/y` 改为“未开始隐藏、开始后入场、结束后短留再淡出”。
  - Why：对齐“开始前不要出现，开始后再来且有动效”的交互要求。
  - Impact：`app/today/page.tsx`、`app/globals.css`。
  - Verify：初始不显示；开始后带动效出现；结束后保留约 2 秒再淡出；`npm run lint && npm run test && npm run typecheck && npm run build` 通过。

## Status / Next（增量）

- **[2026-02-15] 当前状态**：顶部进度区显隐与 Liquid Glass 动效已完成并通过本地全量校验。
- **[2026-02-15] 下一步**：按既定流程重部署大陆测试 VPS，验收 `today` 页开始/结束时机与动效手感。

## Decisions（增量）

- **[2026-02-15] Today 首页同层同色化**：中心页头部与主区改为同层同色背景，并移除中段分割线。
  - Why：按最新反馈统一视觉层级，避免“颜色不一致 + 分割线”割裂感。
  - Impact：`app/today/page.tsx`、`app/globals.css`。
  - Verify：首页 `Pomlist` 标题区与内容区颜色一致，不再出现中间分割线；`npm run lint && npm run test && npm run typecheck && npm run build` 通过。

## Status / Next（增量）

- **[2026-02-15] 当前状态**：同层同色与去分割线已完成并通过本地全量校验。
- **[2026-02-15] 下一步**：在大陆测试 VPS 重部署并验收 `today` 页视觉一致性。
